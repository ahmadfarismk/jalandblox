...
| 2026-09-21 | X2 | OpenTripMap as a second real source for See, a three-way source picker, the `VITE_NEARBY_SOURCE` rename, and a distance-based duplicate filter so a landmark under another name is no longer listed twice |
...
50. **A second source for the nearby spike, and the duplicate fix** (Syakir, spike X2, 2026-09-21). `src/guide/nearby/opentripmap.js` is a second provider behind the same four functions and the same result shape, so the debug page can switch between **fake**, **geoapify** and **opentripmap**. It is wired up for the **See** group only and says so plainly for Eat and Stay. **The switch is renamed `VITE_NEARBY_SOURCE`** now that there is more than one provider; the old `VITE_GEOAPIFY_SOURCE` is still read as a fallback so no existing `.env` breaks. **Faris: `.env.example` needs `VITE_NEARBY_SOURCE=fake` and `VITE_OPENTRIPMAP_KEY=` (name only), and `VITE_GEOAPIFY_SOURCE` can go.** I did not edit that file. `ourPlaces.js` now drops any result within **150 m** of one of our 7 when the categories overlap, as well as the old exact-name match: Geoapify was returning our landmarks as "Menara Berkembar Petronas" and three separate copies of "Bangunan Sultan Abdul Samad", which the name match missed. Known cost of that filter: it also drops real distinct places that sit right next to ours, such as Petrosains and Dataran Merdeka. Still a hidden debug page, still nothing wired into the app, still no new libraries. (Everyone.)
